...
// / ArrString is a helper routine that allocates a new []string value
// to store v and returns a pointer to it.
func ArrString(v []string) *[]string { return &v }
...
// PtrToPtr is a helper routine that returns a pointer to a pointer.
func PtrToPtr[Value any](v *Value) *Value {
	return v
}

// create function, with any, if that data is default value, we will convert to nil
func ConvertToNil[Value comparable](v Value) *Value {
	var defaultData Value
	if v == defaultData {
		return nil
	}
	return &v
...
// ConvertArrPtrUuidToStringArr convert array of pointer uuid to array of string
func ConvertArrPtrUuidToStringArr(arr *[]uuid.UUID) *[]string {
	out := make([]string, 0)
	if arr == nil {
		return nil
	}
...
// CopyInt16 copies the value from source pointer to destination pointer.
// If source is nil, no operation is performed.
func CopyInt16(source *int16, dest **int16) {
	if source == nil {
		return
	}
	value := *source
...
// CopyFloat64 copies the value from source pointer to destination pointer.
// If source is nil, no operation is performed.
func CopyFloat64(source *float64, dest **float64) {
	if source == nil {
		return
	}
	value := *source
